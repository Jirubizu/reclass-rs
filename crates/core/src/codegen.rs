...
fn main_hook(reg: &ClassRegistry, root: Option<ClassId>, attach: &str) -> String {
    use std::fmt::Write;
    let mut out = String::new();
    out.push_str(
    ...
    match root {
        Some(id) => {
            let view = format!("{}View", class_type_name(reg, id));
            out.push_str("use generated::*;\n");
            out.push_str("use vmem::Process;\n\n");
            let _ = write!(
                out,
                "fn main() -> Result<(), Box<dyn std::error::Error>> {{\n\
                 \x20   // 1. Attach to the target process (by name or pid).\n\
                 \x20   let proc = Process::by_name({attach:?})?;\n\
                 \x20   // let proc = Process::by_pid(0)?;\n\n\
                 \x20   // 2. Resolve the base address of your root class,\n\
                 \x20   //    e.g. a module base plus a static offset:\n\
                 \x20   //    let base = proc.module({attach:?})?.base + 0x0;\n\
                 \x20   let base: usize = 0;\n\n\
                 \x20   // 3. Bind an accessor, then read/write fields.\n\
                 \x20   let view = {view}::new(&proc, base);\n\
                 \x20   let _ = &view;\n\
                 \x20   // let value = view.some_field()?;\n\
                 \x20   // println!(\"{{value:?}}\");\n\n\
                 \x20   Ok(())\n\
                 }}\n"
            );
        ...
        None => {
            ...
            let _ = write!(
                out,
                "fn main() -> Result<(), Box<dyn std::error::Error>> {{\n\
                 \x20   // No classes defined in the reclass project yet.\n\
                 \x20   let _proc = Process::by_name({attach:?})?;\n\
                 \x20   Ok(())\n\
                 }}\n"
            );
        }
